...
import { api, type AccessRole, type WorkspaceUser } from "../utils/api";
import { Badge, Btn, Field, Modal, Spinner, inputCls } from "./ui";

type UserStatus = WorkspaceUser["status"];
type Audit = { id: string; at: string; actor: string; action: string; subject: string };

const AUDIT_STORAGE = "dataforge-admin-audit";
const ROLES: AccessRole[] = ["Administrator", "Developer", "Analyst", "Viewer"];

function read<T>(key: string, fallback: T): T {
  try { return JSON.parse(localStorage.getItem(key) ?? "") as T; } catch { return fallback; }
}

const roleTone = (role: AccessRole): "accent" | "ok" | "warn" | "neutral" =>
...
function UserForm({
  ...
  user?: WorkspaceUser;
  onSave: (v: { name: string; email: string; role: AccessRole; mfa: boolean; password?: string }) => Promise<string | void>;
  onClose: () => void;
}) {
  ...
  const submit = async () => {
    ...
    if (!user && password.length < 8) return setError("Set a password of at least 8 characters.");
    if (password && password.length < 8) return setError("Password must be at least 8 characters.");
    setSaving(true);
    const err = await onSave({ name: name.trim(), email: email.trim().toLowerCase(), role, mfa, ...(password ? { password } : {}) });
    setSaving(false);
    if (err) setError(err);
  };
  return (
    <Modal title={user ? `Edit ${user.name}` : "Add Dataforge user"} onClose={onClose} width={480}>
      <div className="space-y-3.5">
        ...
        <Field label={user ? "New password (leave blank to keep current)" : "Password"}>
          <input type="password" className={inputCls} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={user ? "••••••••" : "At least 8 characters"} />
        </Field>
        <Field label="Role">
          <select className={inputCls} value={role} onChange={(e) => setRole(e.target.value as AccessRole)}>{ROLES.map((r) => <option key={r}>{r}</option>)}</select>
        </Field>
        <label className="flex items-center gap-2.5 rounded-lg border border-bdrsoft bg-panel2/50 px-3 py-2.5 cursor-pointer">
          ...
          <span className="text-[12px] text-soft"><span className="font-medium text-ink">Require multi-factor authentication</span><br />Recorded on the account; Dataforge does not prompt for MFA itself yet.</span>
        </label>
        {error && <p className="text-[11.5px] text-err">{error}</p>}
        <div className="flex justify-end gap-2 pt-1"><Btn variant="outline" onClick={onClose}>Cancel</Btn><Btn variant="primary" onClick={submit} disabled={saving}>{saving ? "Saving…" : user ? "Save changes" : "Create user"}</Btn></div>
      </div>
    </Modal>
  );
}
...
export default function UserAdmin() {
  ...
  const [formUser, setFormUser] = useState<WorkspaceUser | undefined | null>(null);

  const load = () => {
    api.users().then((r) => setUsers(r.users)).catch((e: Error) => setLoadError(e.message));
  };
  useEffect(load, []);
  ...
  const save = async (value: { name: string; email: string; role: AccessRole; mfa: boolean; password?: string }): Promise<string | void> => {
    try {
      if (formUser) {
        const { user } = await api.updateUser(formUser.id, { ...value, status: formUser.status });
        setUsers((prev) => (prev ?? []).map((u) => (u.id === user.id ? user : u)));
        log("Updated user", user.name);
        s.toast("success", `${user.name}'s access updated`);
      } else {
        const { user } = await api.createUser(value);
        setUsers((prev) => [...(prev ?? []), user]);
        log("Added user", user.name);
        s.toast("success", `${user.name} added`);
      }
      s.refreshSession();
      setFormUser(null);
    } catch (e) {
  ...
  const changeStatus = (user: WorkspaceUser) => {
    const nextStatus: UserStatus = user.status === "Active" ? "Suspended" : "Active";
    s.askConfirm({
      title: `${nextStatus === "Suspended" ? "Suspend" : "Restore"} ${user.name}?`,
      body: nextStatus === "Suspended" ? "They will immediately lose access to Dataforge. Their role is retained." : "They will regain access with their existing role.",
      confirmLabel: nextStatus === "Suspended" ? "Suspend user" : "Restore user",
      danger: nextStatus === "Suspended",
      onConfirm: async () => {
        try {
          const { user: updated } = await api.setUserStatus(user.id, nextStatus);
          setUsers((prev) => (prev ?? []).map((u) => (u.id === updated.id ? updated : u)));
          log(nextStatus === "Suspended" ? "Suspended user" : "Restored user", user.name);
          s.toast("info", `${user.name} ${nextStatus.toLowerCase()}`);
          s.refreshSession();
        } catch (e) {
  ...
  const remove = (user: WorkspaceUser) => s.askConfirm({
    title: `Remove ${user.name}?`,
    body: "This permanently removes the user from Dataforge. This action cannot be undone.",
    ...
    onConfirm: async () => {
      try {
        ...
        s.toast("info", `${user.name} removed`);
        s.refreshSession();
      } catch (e) {
        s.toast("error", (e as Error).message);
      }
  ...
  const filtered = useMemo(
    () => (users ?? []).filter((u) => (roleFilter === "All" || u.role === roleFilter) && `${u.name} ${u.email}`.toLowerCase().includes(query.toLowerCase())),
  ...
  if (!users) {
    return (
      <div className="h-full grid place-items-center text-[12px] text-mute">
        {loadError ? <span className="text-err">{loadError}</span> : <Spinner label="Loading users…" />}
      </div>
    );
  }
  ...
  return <div className="h-full overflow-y-auto p-4 space-y-4 df-fade">
    <div className="flex flex-wrap items-center gap-2.5">
      ...
      <div><h2 className="text-[14px] font-semibold">Admin</h2><p className="text-[11.5px] text-mute">Workspace accounts — roles here are enforced by the server on every request, not just this screen.</p></div>
    ...
    <section className="border border-bdr rounded-xl bg-panel overflow-hidden">
      <div className="px-4 pt-3 border-b border-bdrsoft flex flex-wrap items-center gap-3">
        <div className="flex gap-4 self-stretch"><button className={`text-[12px] font-medium border-b-2 ${view === "users" ? "border-accent text-ink" : "border-transparent text-mute"}`} onClick={() => setView("users")}>Users</button><button className={`text-[12px] font-medium border-b-2 ${view === "audit" ? "border-accent text-ink" : "border-transparent text-mute"}`} onClick={() => setView("audit")}>Recent activity {audit.length > 0 && <span className="text-mute">({audit.length})</span>}</button></div>
        {view === "users" && <div className="ml-auto flex gap-2 pb-2.5"><div className="relative"><Search size={13} className="absolute left-2.5 top-2 text-mute" /><input className="h-7 w-52 pl-7 pr-2 rounded-md bg-panel2 border border-bdr text-[11.5px] focus:outline-none focus:border-accent" placeholder="Search users" value={query} onChange={(e) => setQuery(e.target.value)} /></div><select aria-label="Filter by role" className="h-7 rounded-md bg-panel2 border border-bdr px-2 text-[11.5px]" value={roleFilter} onChange={(e) => setRoleFilter(e.target.value as "All" | AccessRole)}><option>All</option>{ROLES.map((r) => <option key={r}>{r}</option>)}</select></div>}
      </div>
      {view === "users" ? <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-panel2/60 text-[10.5px] uppercase tracking-wider text-mute"><tr><th className="px-4 py-2.5 font-semibold">User</th><th className="px-3 py-2.5 font-semibold">Role</th><th className="px-3 py-2.5 font-semibold">Security</th><th className="px-3 py-2.5 font-semibold">Created</th><th className="px-3 py-2.5 font-semibold">Status</th><th className="px-4 py-2.5" /></tr></thead><tbody>{filtered.map((u) => <tr key={u.id} className="border-t border-bdrsoft hover:bg-panel2/35"><td className="px-4 py-3"><div className="flex items-center gap-2.5"><div className="w-7 h-7 rounded-full bg-accentdim text-accenthi grid place-items-center text-[10px] font-bold">{u.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}</div><div><div className="text-[12.5px] font-medium">{u.name}</div><div className="text-[11px] text-mute">{u.email}</div></div></div></td><td className="px-3 py-3"><Badge tone={roleTone(u.role)}>{u.role}</Badge></td><td className="px-3 py-3">{u.mfa ? <span className="inline-flex items-center gap-1 text-[11px] text-ok"><BadgeCheck size={13} /> MFA enabled</span> : <span className="text-[11px] text-warn">MFA not enrolled</span>}</td><td className="px-3 py-3 text-[11.5px] text-soft whitespace-nowrap">{new Date(u.createdAt).toLocaleDateString()}</td><td className="px-3 py-3"><Badge tone={u.status === "Active" ? "ok" : "err"}>{u.status}</Badge></td><td className="px-4 py-3"><div className="flex justify-end gap-1"><Btn title="Edit user" onClick={() => setFormUser(u)}><Pencil size={13} /></Btn><Btn title={u.status === "Active" ? "Suspend user" : "Restore user"} onClick={() => changeStatus(u)}>{u.status === "Active" ? <UserMinus size={13} /> : <UserRoundCheck size={13} />}</Btn><Btn title="Remove user" variant="ghost" onClick={() => remove(u)} className="hover:!text-err"><Trash2 size={13} /></Btn></div></td></tr>)}</tbody></table>{filtered.length === 0 && <div className="py-12 text-center text-[12px] text-mute">No users match this filter.</div>}</div> : <div className="divide-y divide-bdrsoft">{audit.length ? audit.map((item) => <div key={item.id} className="px-4 py-3 flex gap-3 items-center"><div className="w-7 h-7 shrink-0 rounded-full bg-panel2 text-accenthi grid place-items-center"><Activity size={13} /></div><div className="text-[12px]"><span className="font-medium">{item.actor}</span> <span className="text-soft">{item.action.toLowerCase()}</span> <span className="font-medium">{item.subject}</span></div><time className="ml-auto text-[11px] text-mute whitespace-nowrap">{item.at}</time></div>) : <div className="py-14 text-center text-[12px] text-mute"><Clock3 size={18} className="mx-auto mb-2" />Actions taken in this browser will appear here.</div>}</div>}
    </section>
