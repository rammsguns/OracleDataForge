...
export default function RowEditor({ table, connId }: { table: string; connId: string }) {
  ...
  const saveEdit = useCallback(() => {
    ...
    void apply({ table: data.table, action: "update", rowId: editing.rowId, values });
  }, [apply, columns, data, editing, s]);

  const saveNew = useCallback(() => {
    if (!adding || !data) return;
    const values: Record<string, string | null> = {};
    for (const [name, text] of Object.entries(adding)) values[name] = text === "" ? null : text;
                    ...
                    <div className="flex gap-1">
                      {isEditing ? (
                        ...
                        <>
                          ...
                          <Btn
                            variant="outline"
                            onClick={() => void apply({ table: data.table, action: "delete", rowId })}
                            disabled={busy}
